defmodule Api.RoomControllerTest do
  use Api.ConnCase
  ...
  defp create_test_rooms(user) do
    ...
    Repo.insert! %Api.Room{owner_id: user.id, name: name}
    end

    # Create two rooms owned by another user
    other_user = Repo.insert! %Api.User{}
    Enum.each ["fourth room", "fifth room"], fn name -> 
    Repo.insert! %Api.Room{owner_id: other_user.id, name: name}
    end
  end


  setup %{conn: conn} do
    user = Repo.insert! %Api.User{}
